...
`assets/logo.jpeg` is the source. The two files under `static/img/` are
derived from it and regenerated by:

```bash
docker compose exec web python scripts/build_logo.py
```

The script cuts the mark out of the paper it was rendered on and writes
`logo-160.png` (the full mark, used on the login page and inlined into the
portfolio PDF — RFP §10.3) and `mark-96.png` (the arc and children without
the wordmark, for the 40px sidebar tile, where the lettering is unreadable).
Replace `logo.jpeg` and re-run it rather than editing the PNGs by hand.
